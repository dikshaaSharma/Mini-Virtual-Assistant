...
def record_audio(ask=False):  # user voice record
    with sr.Microphone() as source:
         ...
         try:
             ...
             print('Oops something went Wrong')  # lee_voice('Oops something went Wrong')
         return voice_data


def hey_voice (audio_string):  # Play audio text to voice
...
class Widget:
    #GUI OF VIRTUAL ASSISTANT AND COMMANDS GIVEN
      def __init__(self):
        ...
        img = ImageTk.PhotoImage(Image.open("C:\Users\Diwakar\PycharmProjects\pythonProject4\venv"))
...
# compFrame.pack(fill="both", expand='yes')
        btn = Button(root, text='Speak', font=('railways', 10, 'bold'), bg='red', fg='white', command=self.clicked).pack(
    fill='x', expand='no')
        btn2 = Button(root, text='Close', font=('railways', 10, 'bold'), bg='yellow', fg='black', command=root.destroy).pack(
    fill='x', expand='no')
        hey_voice('How can i help you ?')
        root.mainloop()
      ...
      def clicked(self):  # BUTTON CALLING
           ...
           if 'who are you' in voice_data:
               hey_voice('My name is hey ')
           if 'search' in voice_data:
               search = record_audio('What do you want to search for ?')
               url = 'https://google.com/search?q=' + search
               webbrowser.get().open(url)
               hey_voice('Here is what i found' + search)

           if 'find location' in voice_data:
               location = record_audio('What is your location?')
               url = 'https://google.nl/maps/place/' + location + '/&amp;'
               webbrowser.get().open(url)
               hey_voice('Here is location' + location)

           if 'what is the time' in voice_data:
               hey_voice("Sir the time is :" + ctime())

           if 'exit' in voice_data:
               hey_voice('Thanks have a good day ')
               exit()
